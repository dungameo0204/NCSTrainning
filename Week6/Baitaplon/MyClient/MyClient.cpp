...
void SingleReceiverThread() {
	...
	while (true) {
		...
		// [ĐÃ SỬA] BÁO CÁO NGHẼN MẠNG VÀO THẲNG CỬA SỔ CON
		if (h.type == MSG_FLOW_CONTROL) {
			auto* pfc = reinterpret_cast<PayloadFlowControl*>(b.data());

			std::lock_guard<std::mutex> lock(g_ClientMutex);
			// Lặp qua tất cả các tiến trình đang chạy và bơm dòng cảnh báo vào ống nước của tụi nó
			...
			continue; // Đọc xong vứt gói tin, nhảy sang vòng lặp tiếp
		}
		...
		if (h.type == MSG_SCAN_RESP) {
			...
			std::lock_guard<std::mutex> lock(g_ClientMutex);
			// NẾU TIẾN TRÌNH KHÔNG CÓ TRONG SỔ (BỊ ÉP RÚT PHÍCH CẮM) -> VỨT RÁC NGAY VÀ LUÔN!
			if (g_ActiveJobs.find(status->jobId) == g_ActiveJobs.end()) continue;
			...
			if (status->status == 2 || status->status == 3) {
				if (job.hPipeWrite != INVALID_HANDLE_VALUE) {
					...
					// [MỚI] TẠO "HÓA ĐƠN" TỔNG KẾT NẾU QUÉT XONG
					if (status->status == 2) {
						finalMsg = L"\n==========================================\n";
						...
						// Nếu bị Hủy thì in ngắn gọn thôi
						finalMsg = L"\n[X] DA HUY TIEN TRINH!\n";
					}

					// Bơm hóa đơn vào ống nước cho Cửa sổ con in ra
					DWORD w;
...
int main() {
	...
	if (argc >= 4 && wcscmp(argv[1], L"--monitor") == 0) {
		...
		while (ReadFile(hPipe, buf, sizeof(buf) - sizeof(wchar_t), &bytesRead, NULL) && bytesRead > 0) {
			...
			// TÔ MÀU THEO SỰ KIỆN
			if (output.find(L"[!!!] VIRUS:") != std::wstring::npos) SetColor(12); // Virus -> Đỏ
			else if (output.find(L"[!] WARNING: SERVER QUA TAI") != std::wstring::npos) SetColor(14); // Nghẽn -> Vàng
			else if (output.find(L"[V] QUET HOAN TAT") != std::wstring::npos) SetColor(10); // Xong -> Xanh
			else if (output.find(L"[X] DA HUY") != std::wstring::npos) SetColor(12); // Hủy -> Đỏ
			else SetColor(7); // Bình thường -> Trắng

			wcout << output;
			SetColor(7); // Trả lại màu trắng
		}
			...
			try {
				...
				if (SendPacket(MSG_CANCEL_REQ, &req, sizeof(req))) {
					...
					if (g_ActiveJobs.find(cancelId) != g_ActiveJobs.end()) {
						...
						SetColor(12); wcout << L"[Client] Job " << cancelId << L" Da Huy (Ep Buoc)!" << endl;
						g_ActiveJobs.erase(cancelId); // Gạch tên khỏi sổ
					}
				}
			}
